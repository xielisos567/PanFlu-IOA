...
```
git clone git@github.com:xielisos567/PanFlu-IOA.git
cd PanFlu-IOA
```
...
```
#### or using mamba: mamba create -n panflu python=3.8 && conda activate panflu
...
Output files: Intermediate files (.extract.fasta, .aln.fasta), consensus files (.cons.fasta) and statistics files (.stat.xls) are generated in the output directory (e.g., ./result_mode1). Final results are placed in the final subdirectory.

Expected run time: On a typical 4‑core desktop, using the provided test_data (~10 files, 100 sequences each), Mode 1 with 5 iterations takes about 1‑2 minutes, and Mode 2 about 2‑3 minutes – actual time depends on MAFFT alignment speed.
